...
export function generarComprobanteNomina(res, { periodo, contrato, empresa, trabajador }) {
  ...
  if (complementos.length > 0) {
    doc.moveDown().fontSize(10).fillColor("#333333").text("Desglose de complementos:");
    complementos.forEach((item) => doc.text(`· ${item.concepto || "Complemento"}: ${Number(item.importePz || 0).toLocaleString("es-ES")} Pz (${item.tipo === "actividad" ? "actividad confirmable" : "cargo fijo"})`));
  }
  doc.moveDown();
